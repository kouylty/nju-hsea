...
class EarthBenchEnvUnifiedNGSWindow(gym.Env):
    def __init__(self, p1, p2, reference, dims, f1=None, f2=None, num_segments=None, constraints_dict=None, use_full_mask=True, window_size=1, is_test=False, local_search=False, parent_local_search=False, device='cpu'):
        self.dims = dims
        ...
        if num_segments is None:
            if window_size is None:
                ...
                self.obs_dim = 5 + window_size * 4  # 状态为 (f1, f2, kendall1, kendall2, l, u_vec, v_vec)
        else:
            if window_size is None:
                self.obs_dim = 9 + 2 * num_segments
            else:
                self.obs_dim = 5 + window_size * 4 + 2 * num_segments
        
        if window_size is None:
            self.action_dim = 2582 # 扩展为 2582 个动作（为了所有任务上的统一），超出长度的会 masked
        else:
            self.action_dim = 4 * window_size + 1 # 可行的动作为每个父代中 [-window_size, window_size] 内的整数，以及 随机选择一个点
    ...
    def step(self, action):
        # p1: ..., cur - window_size, cur - window_size + 1, ..., cur - 1, cur, cur + 1, ..., cur + window_size - 1, cur + window_size, ...
        # 动作 a1 对应 cur - window_size, a2 对应 cur - window_size + 1, ..., a_window_size 对应 cur + window_size
    ...
    def _choose(self, action):
        ...
        if 0 <= action < 2 * self.window_size:
            # 对应从 p1 中选择
            # 找到 cur 在 p1 中的位置
            ...
            if 0 <= action < self.window_size:
                # 对应从 p1 当前点左侧选择
                if idx - self.window_size + action < 0:
                    chosen, _ = self._choose_random()
                    return chosen, 0.0 if self.is_test else -5.0
                node = self.p1[idx - self.window_size + action]
            else:
                # 对应从 p1 当前点右侧选择
                if idx + action - self.window_size + 1 >= self.dims:
                    chosen, _ = self._choose_random()
                    return chosen, 0.0 if self.is_test else -5.0
                node = self.p1[idx + action - self.window_size + 1]
        elif 2 * self.window_size <= action < 4 * self.window_size:
            # 对应从 p2 中选择
            idx = np.where(self.p2 == cur)[0]
            assert len(idx) != 0 # 不在 parent 中
            idx = idx[0]
            
            action -= 2 * self.window_size
            if 0 <= action < self.window_size:
                # 对应从 p2 当前点左侧选择
                if idx - self.window_size + action < 0:
                    chosen, _ = self._choose_random()
                    return chosen, 0.0 if self.is_test else -5.0
                node = self.p2[idx - self.window_size + action]
            else:
                # 对应从 p2 当前点右侧选择
                if idx + action - self.window_size + 1 >= self.dims:
                    chosen, _ = self._choose_random()
                    return chosen, 0.0 if self.is_test else -5.0
                node = self.p2[idx + action - self.window_size + 1]
        else:
            # 随机选择一个
            chosen, reward = self._choose_random()
    ...
    def _get_candidates(self):
        ...
        if idx > 0:
            for i in range(self.window_size):
                if idx - self.window_size + i >= 0:
                    candidates.append(self.p1[idx - self.window_size + i])
                else:
                    candidates.append(-1) # 前面没有，用 -1 填充，表示无效
        else:
            candidates.extend([-1] * self.window_size) # 前面没有，用 -1 填充，表示无效

        if idx < len(self.p1) - 1:
            for i in range(self.window_size):
                if idx + i + 1 < self.dims:
                    candidates.append(self.p1[idx + i + 1])
                else:
                    candidates.append(-1) # 后面没有，用 -1 填充，表示无效
        else:
            candidates.extend([-1] * self.window_size) # 后面没有，用 -1 填充，表示无效
        ...
        if idx > 0:
            for i in range(self.window_size):
                if idx - self.window_size + i >= 0:
                    candidates.append(self.p2[idx - self.window_size + i])
                else:
                    candidates.append(-1) # 前面没有，用 -1 填充，表示无效
        else:
            candidates.extend([-1] * self.window_size) # 前面没有，用 -1 填充，表示无效
        if idx < len(self.p2) - 1:
            for i in range(self.window_size):
                if idx + i + 1 < self.dims:
                    candidates.append(self.p2[idx + i + 1])
                else:
                    candidates.append(-1) # 后面没有，用 -1 填充，表示无效
        else:
            candidates.extend([-1] * self.window_size) # 后面没有，用 -1 填充，表示无效
...
if __name__ == "__main__":
    ...
    ref = np.random.permutation(dim)

    window_size = 4
    env = EarthBenchEnvUnifiedNGSWindow(p1, p2, ref, dims=dim, num_segments=16, window_size=window_size)
    obs, info = env.reset()
    done = False
    total_reward = 0
