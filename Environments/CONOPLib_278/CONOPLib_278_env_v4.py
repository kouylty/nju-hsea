...
class EarthBenchEnv(gym.Env):
    ...
    def _calculate_final_reward(self):
        ...
        # 这里根据解的绝对质量赋予不同的奖励权重，激励模型向高质量解靠近
        # 奖励函数：
        # 如果有提升 w(f) * (f - best_f)
        # 没有则还是 f - best_f
        advantage = fitness - self.best_f
        return advantage if advantage < 0 else self.w(fitness) * advantage

    def _choose_from(self, parent):
        candidates = parent[:self.step_count+1]
    ...
    def _choose_random(self):
        ...
        remaining = [i for i in self.best_p if i not in self.o_partial]
