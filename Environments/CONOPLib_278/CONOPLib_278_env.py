...
class EarthBenchEnv(gym.Env):
    ...
    def _calculate_final_reward(self):
        ...
        return (fitness - self.best_f) / 200 + reward
    ...
    def _choose_random(self):
        ...
        if not valid:
            ...
            return None, -50.0  
        return np.random.choice(valid), 1.0
...
# class EarthBenchEnv(gym.Env):
#     def __init__(self, p1, p2, dims, f1=None, f2=None, constraints_dict=None):
#         self.dims = dims
#         self.obs_dim = dims * 3
#         self.action_dim = 3 # 只有 3 个动作，即从父代1的边中选择，从父代2的边中选择，从所有边中随机选择一个
...
#         done = (self.step_count >= self.dims)
#         if done:
#             reward = self._calculate_final_reward()

#         return self._get_obs(), reward, done, False, {} # (obs, reward, terminated, truncated, info)

#     def _calculate_final_reward(self):
#         fitness = EarthBenchmark_278()(self.o_partial)
#         reward = kendall_ranking_correlation(self.p2, self.o_partial, device='cpu').item() * 500
#         # print(f"Final fitness: {fitness}, f1: {self.f1}, f2: {self.f2}, best_f: {self.best_f}, better? {fitness > self.best_f}")
#         return (fitness - self.best_f) + reward
